...
// PreConsumeUserSubscriptionTx is the transaction-aware form used when a
// task reservation, its idempotency event, and the subscription debit must be
// committed atomically. The caller owns the transaction lifecycle.
...
// RefundSubscriptionPreConsumeTx is the transaction-aware counterpart of
// RefundSubscriptionPreConsume. It keeps the task refund event and the
// subscription credit in the same primary-database transaction.
...
// the table small. Records backing an unsettled durable task stay available so
// a delayed failure can still be refunded after polling has been paused.
func CleanupSubscriptionPreConsumeRecords(olderThanSeconds int64) (int64, error) {
	if olderThanSeconds <= 0 {
		olderThanSeconds = 7 * 24 * 3600
	}
	cutoff := GetDBTimestamp() - olderThanSeconds
	activeTaskReserveEvents := DB.Model(&TaskBillingEvent{}).
		Select("event_key").
		Joins("JOIN tasks ON tasks.id = task_billing_events.task_id").
		Where("task_billing_events.type = ?", TaskBillingEventReserve).
		Where("tasks.billing_status IN ?", []TaskBillingStatus{TaskBillingStatusReserved, TaskBillingStatusRefundPending})
	res := DB.Where("updated_at < ?", cutoff).
		Where("status != ? OR request_id NOT IN (?)", "consumed", activeTaskReserveEvents).
...
// PostConsumeUserSubscriptionDeltaTx updates subscription usage inside an
// existing transaction. The caller owns commit/rollback.
